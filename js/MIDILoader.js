...
(function () {
  ...
  widgets.Loader = function (configure) {
    ...
    this.add = function (conf) {
      ...
      var background = configure.background
...
";
    ...
    var removeChild = function (item) {
      window.setTimeout(function () {
        // timeout in case within same event loop.
        item.container.style.opacity = 0;
      }, 1);
      window.setTimeout(function () {
        // wait for opacity=0 before removing the element.
      ...
      for (var key in that.messages) {
        ...
        if (nid % 5 === 0 && item.getProgress) {
          if (
            item.timeout &&
            item.timestamp &&
            timestamp - item.timestamp > item.timeout
          ) {
